...
class Lambdas(Construct):
    def __init__(self, scope: App, id: str, **kwargs) -> lamb.Function:
        super().__init__(scope, id, **kwargs)

    def lam_upload(self) -> lamb.Function:
        ...
        return lambda_upload

    def lam_download(self) -> lamb.Function:
        # Creating Lambda functions using the code from ./lambda/
...
class ApiGateway(Stack):
    def __init__(self, scope: Construct, id: str, **kwargs) -> None:
        ...
        lambda_upload_int = apigw.LambdaIntegration(
            Lambdas.lam_upload(self),
            proxy=True,
        )

        lambda_download_int = apigw.LambdaIntegration(
            Lambdas.lam_download(self),
            proxy=True,
        )
        ...
        # # Cloudwatch alarms
        # lambda_alarm_metric_upl=Lambdas.lam_upload(self).metric_all_errors(label="LambdaUploadErrors",
        #                                                     period=Duration.seconds(60),
        #                                                     statistic="sum")
        # lambda_alarm_metric_dl=Lambdas.lam_download(self).metric_all_errors(label="LambdaDownloadErrors",
        #                                                     period=Duration.seconds(60),
        #                                                     statistic="sum")
        #
